...
### 2026-09-24 — sitemap.xml had zero per-town berdekatan.html entries, unnoticed
PROBLEM: While scoping which prerender routes deserved an English (`?lang=en`) sitemap entry, found that `berdekatan.html` had NO per-town entries in `api/sitemap.js` at all -- only the bare static `/berdekatan.html` URL, even though the page has served real per-town "near me" content (and ranked/earned clicks, going by the kawasan precedent M23 already documented) since it was built. Nobody had asked for this specifically; it surfaced as a side effect of auditing the file for an unrelated, smaller change.
WORKED: Reused the existing `allKawasan` town list (already built for kawasan's entries) for berdekatan too, rather than a second town/label fetch -- justified because `renderBerdekatan()` in `api/prerender.js` is deliberately the same town/neighbourhood matcher as `renderKawasan()` (see that function's own comment), so the two pages share an identical valid-town population by construction. One list now drives four entry sets (kawasan ms/en, berdekatan ms/en).
FAILED: nothing tried and reverted -- first approach (share the list) held.
RULE: When auditing any generated file (sitemap, feed, index) for one targeted change, check its neighboring sections for the same class of gap before calling the audit done -- a missing feature in one of several structurally-parallel sections (here: kawasan had per-town entries, berdekatan didn't) is easy to miss by reading top-to-bottom instead of comparing siblings.
ROUTED TO: api/sitemap.js itself (fixed in place, comments explain the shared-list reasoning); no CLAUDE.md/skill update needed -- this is a one-off completeness gap, not a recurring mistake class.

### 2026-09-24 — A vercel.json rewrite that fires can still drop query params silently
PROBLEM: Extended `?lang=en` AI-crawler support to school.html (following the same fix already shipped for kawasan.html/berdekatan.html earlier the same day, M70). `/school/:slug`'s bot rewrite was never shadowed by a static file the way kawasan/berdekatan's were, and a live `curl -A "ClaudeBot" ".../school/<slug>?lang=en"` confirmed it DID reach `/api/prerender` with real content — but the response was Malay every time; `lang=en` never arrived at the function.
WORKED: Folded `/school/:slug` bot routing into `middleware.js` alongside kawasan/berdekatan (which already build the target URL explicitly, param by param, and are proven working), and removed the now-dead bot rule from `vercel.json`, keeping only its plain human-facing rewrite. Dry-run tested locally (stubbed `@vercel/functions`, 7 cases across all three routes/bot-vs-human/with-without lang/empty slug) before shipping; confirmed live after deploy with three curl tests (school EN, school MS, kawasan EN).
FAILED: Nothing tried and reverted, but the near-miss was assuming a rewrite reaching the function meant it was fully correct — reachability and correctness are two separate things to verify.
RULE: A `vercel.json` "destination" rewrite only reliably carries the query params it names — never assume it forwards params it doesn't reference, even when the rule is confirmed reachable. Any route already on Routing Middleware for one reason should have ALL its param-dependent variants handled there too, not split between `middleware.js` and `vercel.json`.
ROUTED TO: CLAUDE.md M71.

### 2026-09-23 — Parent-reported fee schema was undersold by the data-layer skill
PROBLEM: Task asked to write parent-reported (WhatsApp group) fees into `schools`, with field names given only as guesses ("may differ, confirm via information_schema"). The loaded data-layer skill's Fees section listed only `fee_min`/`fee_max`/`monthly_fee (legacy)`, which would have led to inventing `annual_fee`/`registration_fee` as nonexistent columns.
WORKED: Ran `information_schema.columns` first — `annual_fee`, `registration_fee`, and `fee_programs` (jsonb `[{program,amount}]`) all already exist and are actively populated on other rows. Matched 32 WhatsApp-reported schools against `schools` by name+commercial_name ilike plus town/district/state cross-check; only wrote fee fields (with a `fee_programs` per-tier breakdown) to the 7 rows with an unambiguous single-branch match in the right location. Franchise chains with many similarly-named branches and no location clue in the source were left unwritten (MEDIUM/LOW for Fadly's own review), per the same don't-loose-match caution M39/M43/M45 apply to registration numbers.
FAILED: nothing — first approach (verify schema before writing) held.
RULE: Before writing to any table whose field names are given only as a guess, always run `information_schema.columns` first rather than trusting a skill's field list verbatim — skills can undersell or go stale on schema (M26-class staleness applies to column lists too, not just coverage percentages). For fee-style franchise matching, never write to a row unless exactly one plausible branch exists in the queried location, and log every UPDATE's before/after values.
ROUTED TO: CLAUDE.md §2.4 (fee-column list corrected here in the same session); carischool-data-layer skill still needs the same correction next time it's editable from this environment (it lives outside this repo's write access this session — flagging as undischarged).

### 2026-09-04 — A "register new school" flow never required fee, which is how a fully-claimed school ended up with zero fee data, and testing an INSERT-only table's policy the wrong way looked like a false failure
PROBLEM: Comparing two related listings claimed by the same operator (a TASKA and a TADIKA at one premises) surfaced that the TADIKA had no fee data at all, despite fee being an established, explicit product requirement ("fee is very important and compulsory," stated earlier this project). Tracing it: claim.html already correctly requires fee at submission — the actual gap was that the separate "register a new school" flow (daftar-sekolah-baharu.html) never collected fee at all, at any point, so a school approved through that path could be fully claimed with nothing there. Separately, while testing the database fix, `INSERT ... RETURNING id` against `new_school_submissions` (INSERT-only for anon by design) failed with an RLS violation that looked exactly like a broken insert policy — a plain insert without RETURNING succeeded immediately.
WORKED: Added fee_min/fee_max to the submissions table and the daftar-sekolah-baharu.html form itself, with the identical required-at-submission validation claim.html already uses (not a new rule, the same one just applied consistently). Both approval paths (direct-insert and existing-school-claim) updated to carry fee through, including setting fee_updated_at correctly (reusing the freshness-tracking system from earlier the same night) -- tested with real inserted submissions and real approvals before and after, not just read the code. For the RETURNING false alarm: confirmed the real client code never chains .select() after .insert(), so the live site was never actually affected, before concluding anything needed fixing.
FAILED: The first raw-SQL test of the fee-column insert used RETURNING and appeared to fail -- would have been misdiagnosed as a broken column-level grant if not cross-checked against a plain insert (no RETURNING) first, which succeeded immediately and isolated the real cause.
RULE: When a rule is "compulsory," check every entry point that can produce the same end state (claimed school), not just the one that was originally built with the rule in mind -- two parallel flows led to the same is_claimed=true state, and only one of them enforced the rule. Separately: when testing an INSERT-only RLS policy via raw SQL, don't add RETURNING unless the table also grants SELECT -- it fails for a reason unrelated to whether the actual policy works.
ROUTED TO: CLAUDE.md M64 (the RETURNING/RLS mechanism); daftar-sekolah-baharu.html, admin.html, and the database schema (new fee columns + validation, first-hand fix, no dedicated M-number needed beyond the mechanism note).

### 2026-09-03/04 — A real, live cross-school data-tampering vulnerability, the security fix that followed it into a Vercel deployment mystery, and three files that had silently drifted from what was actually shipped
PROBLEM: A third-party audit of kemaskini.html raised, as its top finding, that the page's Supabase writes might not be enforced server-side. Read the actual RLS policy governing school updates and found the audit was right in the most direct way possible: `claimed_schools_can_update_own_profile` checked only `is_claimed = true` on the target row -- nothing tied a write to the specific school whose code had been verified. Confirmed live, not theoretically: used one school's real, correct claim code to update a *different* real school's data, with the change actually landing. The obvious fix (route every write through a function that checks the real claim code first, then close the old open door) surfaced two more problems on the way: `admin.html` had 3 of its own direct writes and roughly 30 total admin-facing database functions with zero identity check of any kind -- including two, found only by deliberately auditing every single one rather than trusting the original short list, that had no protection at all (one could claim any school outright, one could approve any pending claim). And admin.html's own "login" turned out to be a plain client-side string comparison against a password sitting in the page's own source -- real-looking, but disconnected entirely from what the database actually allowed. Building the real fix (a server-side login endpoint, a session-token table, every admin function requiring a valid token) required a new serverless function -- which silently broke every deployment on the project afterward, unrelated files included, for a reason that took real digging to find (Vercel's Hobby-plan 12-function cap, discovered from a build log, not from any error message that named the actual cause). Separately, twice during this stretch, a fresh GitHub clone of a file (admin.html, then claim.html) that had already been extensively fixed and shipped earlier the same session came back missing those fixes entirely -- the person's deploy workflow doesn't push to GitHub in lockstep with what's live.
WORKED: Every fix in this stretch was tested as an actual attack simulation before being trusted -- a wrong code, the correct code, and critically, one school's real code used against a different real school, verified via direct query that the target's data was genuinely untouched each time. The deployment failure was diagnosed from the raw build log (counting compiled function files by hand) rather than guessed at from the generic "Build Failed" UI message, and fixed by merging the new login logic into an existing multi-action endpoint instead of asking for a paid-plan upgrade. The GitHub-drift problem was handled by asking the person to upload the actual current file directly rather than continuing to build on a clone that might be stale, once the pattern repeated a second time.
FAILED: A `CREATE OR REPLACE` on several admin functions with a changed parameter list (adding the new token parameter) created a second overloaded version instead of replacing the first, on more than one function, in the same stretch -- caught each time by an explicit post-write count-of-versions check, not assumed safe from the CREATE OR REPLACE succeeding without error. `create_admin_session()` was built to be callable only server-side but was never explicitly revoked from PUBLIC -- a live SET ROLE anon test (not a read of the function definition) showed anon could call it and mint itself a valid session, which would have completely defeated the point of the fix if shipped as first written.
RULE: A database column-level or policy-level check that verifies "is this row in the right general state" is not the same as verifying "is this specific requester allowed to touch this specific row" -- the two look identical in a policy listing and behave identically until tested as an actual cross-entity attack. Any function meant to be restricted needs its access explicitly revoked and then verified with a real role-switch test, not inferred from the absence of a grant. Before adding a new file under /api/, count what's already there against the hosting plan's limits. And a GitHub clone is not proof of what's live, particularly for a file already fixed once in the same session -- verify or ask before building further on it.
ROUTED TO: CLAUDE.md M62, M63, M65 (Vercel function limit, PostgreSQL PUBLIC-execute default, GitHub-drift); the school-assets storage bucket policy and every admin-facing RPC (first-hand fixes, spread across admin.html, kemaskini.html, and the database schema).

### 2026-08-31 — Same-timing async updates can succeed for one field and fail for its sibling, and a multi-file sweep beats fixing what was reported
PROBLEM: Three external audits (a strategic pitch, panduan.html, index.html) needed each specific claim verified before acting on any of them -- two turned out to be wrong (index.html's "inconsistent alt text" was actually two separate, both-correct images; a cited "4,353" JKM count was the wrong metric). Once verification became the working method, it surfaced something much bigger than any single audit found: live site: searches showed every real indexed school.html page (~11,000+ total) had a correct, unique title but the identical generic static meta description -- even though title and description update one line apart, inside the same async function, gated behind the same Supabase fetch. Same timing, different outcome for sibling fields. Separately, "fix the stale year" tasks kept being narrower than the actual bug -- a single reported field (H1, or schema headline) was often one of 8-10 places the same stale value lived (title, meta description, meta keywords, OG tags, body text, related-card references to other pages, footer copyright), and state.html's template multiplies any fix there across all 16 state pages it generates.
WORKED: Treating "the same timing produced different outcomes" as a real signal, not noise -- concluded this likely isn't a pure race condition (Google is documented to treat descriptions as an overridable "hint" more readily than titles) rather than assuming more `await` would fix it. The fix applied regardless of which mechanism was actually true: a three-tier cascade on school.html (instant slug-based synchronous fallback, a fast narrow parallel fetch, the existing full update) -- each tier independent, none replacing the others, so the page is never worse than before even if one tier fails. For the stale-year sweeps: grep the whole file for the exact stale string before calling a fix done, not just the field that was reported.
FAILED: N/A this entry -- but see the api/prerender.js check below, which is the same "verify before concluding" discipline applied to something that looked broken but wasn't.
RULE: When a value depends on async data, check whether a sibling value on the identical timing succeeded before diagnosing pure timing as the cause. And before "fixing" something that looks like an oversight (Googlebot excluded from the crawler-prerender list), check for a code comment explaining why -- api/prerender.js's own header documented a reasoned decision (position 6-8 already achieved, avoiding Google's cloaking policy), not a gap. Also: lapor.html and preview.html were both already noindex -- checking that first avoids auditing pages that were never meant to rank at all.
ROUTED TO: CLAUDE.md M54, M55, and §2.6 convention 9 (the full audit playbook); carischool-roadmap.md §4 (2026-08-26 to 2026-08-31 entry).

### 2026-08-25 — A UTC-conversion bug that only showed up in the day-by-day bars, not the totals, and a roadmap 3 weeks stale
PROBLEM: kemaskini.html's new activity feed (day-by-day WhatsApp click bars) used .toISOString().slice(0,10) to build local-calendar-day bucket keys. Malaysia is UTC+8, so local midnight always converts to the PREVIOUS day in UTC -- every bucket key landed one day off from the events' own UTC dates, and every bar silently showed 0 despite real clicks existing. The stat boxes above (today/7-day totals) were unaffected since those compare Date objects directly rather than string keys -- caught from a live screenshot showing "totals correct, every bar zero," not from testing. Separately, carischool-roadmap.md hadn't been touched since 2026-08-06 -- three weeks of shipped work (drawer nav, premium photo policy, sustainability dashboard, slug fix, push notifications, activity feed, calculator instrumentation) existed only scattered across chat history and this log, not in the one file meant to be the consolidated source of truth.
WORKED: Rebuilt day-key generation from getFullYear()/getMonth()/getDate() (local, not UTC) consistently on both the seeding and event-bucketing sides -- verified against the exact reported scenario (Node, TZ=Asia/Kuala_Lumpur, same timestamp) before calling it fixed. For the roadmap, added one consolidated dated entry to §4 covering the full sprint rather than five small scattered edits, plus updated the existing fee-coverage thread in §13 with a fresh direct measurement (fee_min/max: 142/11,136 schools, 1.3%; fee_submissions: 0 approved of 38) since that section explicitly invited re-measurement.
FAILED: N/A this entry -- but see the M52 entry (2026-08-22) for the same root pattern (a conversion/formatting function silently answering a different question than intended) showing up twice in one week across unrelated features.
RULE: .toISOString() answers "what UTC instant is this," never "what local calendar day is this" -- for any non-UTC-timezone product, these are different questions with different answers, and the mismatch fails silently (wrong key, no error) rather than loudly. When stat totals and a detail breakdown built from the same underlying data disagree, suspect the breakdown's aggregation logic, not the totals.
ROUTED TO: CLAUDE.md M53; carischool-roadmap.md §4 (new 2026-08-20 to 2026-08-25 entry) and §13 (fee-coverage re-measurement).

### 2026-08-24 — sw.js lived in the wrong directory for how it's actually registered, and push needs it to actually work
PROBLEM: Every page registers the service worker at /sw.js (bare root), but the file lived at api/sw.js -- and every other file under /api/ is a Vercel serverless function expecting a req/res handler, not plain service-worker syntax. No rewrite bridged /sw.js -> /api/sw.js in vercel.json. Passive installability could silently degrade without anyone noticing (the registration call was wrapped in .catch(()=>{})); push notifications can't tolerate that ambiguity since they depend on the service worker actually registering.
WORKED: Moved sw.js to the repo root, alongside manifest.json (which was already correctly placed there). Root-level static serving is unambiguous -- no rewrite needed, matches the one file in this repo that already had this exact placement decision right.
FAILED: N/A -- caught by re-checking where the file actually lived against how it's actually referenced, not by assuming the working registration calls meant the path was correct.
RULE: A silently-caught error (.catch(()=>{}) with no logging) hides broken infrastructure indefinitely. When a new feature's correctness depends on something previously "probably fine," verify the actual file location/routing, don't inherit the assumption.
ROUTED TO: CLAUDE.md §2.6 convention 7.

### 2026-08-24 — First real npm dependency, first package.json, first push notification feature
PROBLEM: Wanted real-time awareness for schools (a WhatsApp click just happened) instead of only a static counter, without building a whole parent-identity/account system first.
WORKED: Scoped to school-side only for phase one -- schools already have claim_code auth, so no new identity concept needed. Reused the existing WhatsApp-click moment as the trigger rather than inventing a new one. web-push (VAPID-signed payload encryption) was the one dependency judged worth breaking the zero-dependency convention for -- everything else in /api/ still uses raw fetch(). iOS Safari can't request push permission outside an installed PWA context (16.4+, home-screen-only) -- this needed its own UI branch, not just a permission prompt that would silently fail there.
FAILED: N/A this session -- but see the sw.js entry above, found while building this.
RULE: A new capability (push) can expose a previously-invisible bug in adjacent infrastructure (sw.js location) that a feature depending only on graceful-degradation (installability) never surfaced. Build the dependency chain deliberately when adding something that can't degrade quietly.
ROUTED TO: CLAUDE.md §2.6 convention 7; package.json (new file, first in repo).

### 2026-08-22 — Mobile nav had zero replacement when it disappeared, and translation key conventions aren't uniform across pages
PROBLEM: index.html's .nav-links{display:none} at ≤600px left every phone visitor with no way to reach Cara Guna/Untuk Sekolah/Jawatan Kosong/Cari Ikut Negeri -- only found by reading the CSS breakpoint, not by any functional check. Separately, while rolling the same hamburger+drawer nav out to 6 more hub pages, copying drawer_filter: (underscore-style) translation keys into a page whose t(id) convention requires the key to equal the element id verbatim would have silently rendered the literal string "drawerFilter" on screen -- caught by checking each page's own applyTranslations function before adding keys, not by audit_i18n.py, which only catches the unwired case.
WORKED: Hamburger + slide-in drawer, added to index (gated ≤600px, since desktop already has .nav-links) and panduan/berdekatan/statistik/kawasan/state/compare (shown always, no desktop alternative exists on those pages). Checked each page's own translation-wiring convention (individual setText(id, t('key')) vs setText(id, t(id)) array loops) before adding keys, matching whichever convention that specific file already used rather than assuming one carries over.
FAILED: First CLAUDE.md edit for this same session decapitated the M51 entry's header line -- an old_str match grabbed only a fragment of M51's text and replaced through it, silently truncating the entry. Caught by grep-verifying the full M51 header string was still present after the edit, not by the edit "succeeding" (it did succeed, just against the wrong span).
RULE: (1) A hidden/removed nav element needs its mobile replacement verified to actually exist, not just that desktop still renders. (2) Never assume a translation-wiring convention carries over from the last file edited -- grep the specific file's own apply-translations function first. (3) After any str_replace on a numbered/sequential doc (M-numbers, ordered lists), grep-verify the entries before and after the edit point are still complete and in order, since a matched fragment can silently span into adjacent content.
ROUTED TO: CLAUDE.md M52; §2.6 convention 6 (drawer recipe).
...
### 2026-09-23 — A read-only "confirmed present" timestamp needs a source per registry, not one global stamp
PROBLEM: Claimed schools stop being actively re-touched after claim (bookmarklet doesn't re-verify), so JKM delisting/expiry could go unnoticed. JKM has jkm_valid_to (a real expiry date); KPM's school_code has no expiry at all, so there was no signal for "has this KPM listing even been checked lately."
WORKED: Added schools.last_registry_verified_at, stamped ONLY by the identifier cross-check already built into Registry Sync (JKM regNo match, MOE code match in AKTIF mode only, SMIPS code match) — never by any profile write, and never for claimed OR unclaimed differently (same stamping logic, the claimed-only filtering happens at report-read time). Built a separate admin tab (Claimed Registry Health) that never mixes claimed with unclaimed, bucketing JKM by real expiry date and KPM by staleness of last_registry_verified_at (6-month threshold, matching Fadly's own manual KPM cadence).
FAILED: Considered auto-emailing schools when JKM expiry is near — rejected for this pass per Fadly's own steer ("only cross check license number for jkm and code for kpm", i.e. registry-identifier verification only, no profile/contact writes) and the standing principle that a computed registration-status claim needs to be proven trustworthy over a cycle before it goes external (see M38). Also rejected stamping last_registry_verified_at on MOE's TUTUP/closed-listing matches — being found in a "closed" listing is not confirmation the school is alive, so stamping there would produce a false "recently verified" reading.
RULE: When an external registry field has no expiry (KPM's school_code), track "last confirmed present" via the identifier-only cross-check that already exists, not via any action that touches the school's profile — and only stamp from listing modes that actually confirm current/active presence, never from a closed/inactive-listing match.
ROUTED TO: CLAUDE.md §2.6 item 10 (new); admin.html Registry Sync (all three sync functions) + new loadClaimedRegistryHealth() tab.

---

### 2026-09-23 — Auto-emailing on a computed registry claim: fine once the framing itself carries the caveat
PROBLEM: Follow-up to the same-day Claimed Registry Health work, which had deferred auto-emailing schools about JKM expiry per M38 ("a computed claim needs to prove itself before it's used to contact a real customer"). Fadly then explicitly asked for the auto-email anyway, fully automatic, no admin review step.
WORKED: Treated this as the person who owns the call reopening a scope he'd deferred, not as a doctrine violation to push back on — M38's real concern is asserting something that could be wrong to a customer, and Fadly's own instruction solved that directly: never assert "your license is expired, renew" — say our records show an expiry date and ask the school to verify with JKM directly in case our data is off. The caution moved from "don't send" to "don't overclaim in what's sent." Built as a cooldown-gated (60-day, my own assumption) group-by-owner-email batch job, sharing api/cron-premium-photo-reversal.js rather than a new file (Vercel's 12-function cap, M62), following the exact same "stamp state only after confirmed send" pattern the file's existing notify half already uses.
FAILED: n/a -- implementation note.
RULE: A prior deferral logged under a stability doctrine (M38) is not permanent policy -- it's the state of a decision until the decision-owner revisits it. When they do, re-examine what the doctrine was actually protecting against (here: false certainty stated to a customer) and solve that specific risk in the new design, rather than treating the earlier deferral as binding forever.
ROUTED TO: CLAUDE.md §2.6 item 11 (new); api/cron-premium-photo-reversal.js (sendJkmExpiryReminders + buildJkmExpiryEmail); schools.jkm_expiry_reminder_sent_at (new column); premium_reversal_runs.jkm_reminded_count/jkm_reminded_schools (new columns).


### 2026-09-23 — Parent-WhatsApp-group fee data written into the "verified" fee fields instead of the crowdsource table
PROBLEM: Given 3 fee-related screenshots plus a long parent-group thread of ~32 schools' self-reported fees, with instruction to update the site's fees by confidence tier. Delegated the matching+write to a general-purpose subagent (told to "read the skill first"). It wrote 7 HIGH-confidence matches directly into schools.fee_min/fee_max/monthly_fee/annual_fee/registration_fee/fee_programs. Fadly caught it: "the source is not from school website. It is from the parents feedback from thread."
WORKED: Confirmed the concern against school.html's own rendering code, which documents fee_min/fee_max as a two-tier "verified" system (claimed-school kemaskini self-submission, or admin curation from the school's own official public fee page) and monthly_fee as a lower "crawled" tier — parent-group hearsay belongs in neither. A separate, purpose-built table (fee_submissions + get_school_fee_estimate() RPC) already exists for exactly this: rendered only when no stronger tier exists, as a "💬 N parent reports" badge with confidence-calibrated copy. Used fee_updated_at timestamps to distinguish genuinely pre-existing data (1 school) from the agent's writes (6 schools were NULL before), reverted all 7 to true prior state, then re-entered the same 8 data points into fee_submissions as status='pending' rows — preserving the research without mislabeling its provenance.
FAILED: Telling a subagent to "read the skill/data-layer docs first" is not sufficient supervision for a write task touching a field whose UI implies a trust level to a real end user (parent reading the site). The orchestrating session (me) did not itself check school.html's fee-rendering code against the plan before approving the writes.
RULE: Before delegating or running any write to a field a user-facing page renders as "verified"/authoritative, actually read that page's rendering code for the provenance contract first — a subagent instruction to "check the docs" does not substitute for the orchestrator verifying the plan matches documented field semantics. When data's real source is informal/crowd-reported, it goes in the crowdsource tier (fee_submissions here), never the verified-tier fields, regardless of match confidence.
ROUTED TO: CLAUDE.md M68 (new); Supabase schools table (7 rows reverted) + fee_submissions (8 rows inserted, status='pending'). Standing process change per Fadly: check CLAUDE.md/learnings-log BEFORE executing a task, not after; this task class (fee-matching from informal sources) not to be delegated to a subagent again.


### 2026-09-24 — Microsoft Clarity AI-citation reports: a new signal, and the playbook for acting on one
PROBLEM: Fadly started pulling Clarity's "AI Visibility" reports (Share of Authority / citations by query, tracked as topic clusters like "Preschool selection guidance") — a new data source this codebase's docs didn't cover at all. First 7-day and 30-day pulls showed strong, growing SoA on Malay compliance/registration queries (100% on a few), but a separate topic-cluster report showed 0% CariSchool citations across 13 English-language "how to choose a preschool" prompts, with "No top content available."
WORKED: Diagnosed the 0% before assuming a content gap — checked `cara-pilih-tadika.html` directly and found it already covers most of the same substance, just entirely in Malay (title/meta/H1/body). Every site winning that English cluster (naeyc.org, greatschools.org, psychologytoday.com) is English. Reframed it as a language-matching gap, not a content-quality gap, which changed the fix from "write new content from scratch" to "localize/re-target one specific angle." Wrote a content brief prioritizing only the prompts CariSchool can realistically win (the Malaysia-specific wedge prompt, registration/accreditation, teacher ratios, fees — all backed by existing pages/data) and explicitly skipping generic pedagogy questions that are big US nonprofit/authority-site territory. Built the new page as a standalone English guide (not a bilingual retrofit of the Malay page) specifically so the already-ranking Malay asset stayed untouched — a real, escalated content-strategy decision (new page vs. retrofit), not something executed silently.
FAILED: Told Fadly the three related Malay guides had been updated with a link back to the new page. They hadn't — see CLAUDE.md M69. Caught only because Fadly went looking for the page on the live site and couldn't find it, not because anything I did surfaced the gap myself.
RULE: (1) When a citation/ranking report shows a total absence for a whole query cluster, check whether relevant content already exists in a different form (language, framing) before treating it as a from-scratch content gap. (2) Never report a multi-file edit as complete without diffing every file it was supposed to touch — see M69. (3) For future Clarity pulls: 7-day windows are too thin (single-digit citation counts flip a lot); 30-day is the right default cadence, 90-day for trend-watching. Report the raw citation count alongside SoA %, since SoA moves with competitors' citation counts too, not just CariSchool's own.
ROUTED TO: CLAUDE.md M69 (new); `how-to-choose-a-preschool-malaysia.html` (new page); related-strip links added to `cara-pilih-tadika.html`, `kpm-vs-jkm-tadika-taska.html`, `yuran-tadika-malaysia.html`. Not yet routed: `carischool-roadmap.md` — this is a genuinely new traffic-acquisition channel worth a section there, but the file is 120KB and wasn't read this session, so I didn't blind-write to it; flagging as open for Fadly or a future session with roadmap context loaded.


### 2026-09-24 — Vercel serves a matching static file BEFORE evaluating a vercel.json rewrite
PROBLEM: kawasan.html and berdekatan.html's bot-UA `has`-conditioned rewrite rules in vercel.json never actually fired in production, likely since they were first added -- confirmed via live `curl -A "ClaudeBot"` tests returning the plain static SPA shell regardless of UA or query params, while calling `/api/prerender` directly (bypassing vercel.json) returned correct output. Both paths are real static files sitting in the repo, and Vercel serves an exact-path static-file match before the rewrites stage runs at all, so the `has` condition was never even evaluated.
WORKED: A project-root `middleware.js` (Vercel Routing Middleware, `@vercel/functions`' `rewrite()`/`next()`) runs *before* static file serving and replaces the dead vercel.json rules. Root placement is required for two independent reasons: it's where Vercel's spec says middleware must live, AND practically -- a copy uploaded into `/api/` by mistake counted as a 13th Serverless Function and hard-failed every deployment on the Hobby plan's 12-function cap (confirmed directly via the Vercel API: `errorCode: "exceeded_serverless_functions_per_deployment"`, not guessed). GitHub's web-upload UI also silently placed files in whatever folder it was last navigated into, twice, in different directions (a middleware.js landing in api/, then later a prerender.js landing at repo root) -- each one confirmed and caught only by checking `git ls-tree origin/main:<path>` and the actual deployment result, not by assuming an upload succeeded correctly.
FAILED: First attempt (same session) added a more specific vercel.json rewrite rule (the `lang=en` variant) on top of the already-dead rule -- valid JSON, deployed clean, still never reached; only a live curl test with the real bot UA caught that it was dead code, not deploy success, not JSON validation, not local logic tests.
RULE: Never trust a `has`-conditioned vercel.json rewrite whose `source` exactly equals a real static file's path -- it will not fire, however correct its `has` conditions look. Verify any condition-gated rewrite (bot UA, query param, header) with a live fetch matching the real condition before calling it done, not deploy success. New Routing Middleware must be placed at the project root, never under `/api/` -- and after any GitHub web-upload of multiple files, verify each file's actual path/content on GitHub directly (`git ls-tree`/diff), never assume the upload dialog put files where intended.
ROUTED TO: CLAUDE.md M70 (new); middleware.js (new file); vercel.json (dead kawasan.html/berdekatan.html bot rules removed); api/prerender.js (renderKawasan/renderBerdekatan now also carry the `?lang=en` English branch added the same session, itself routed as a smaller M36-adjacent content-parity note, not a separate M-number).


### 2026-09-25 — Expired-license schools need a pending-status badge, not a silent premium downgrade
PROBLEM: Taska Permata Alesha stayed premium 19+ months after its JKM license (`jkm_valid_to`) lapsed. Standing policy needed: expired/expiring license ⇒ verified-claim-only, never premium, shown on the profile card. JKM has a real expiry column; KPM/TADIKA has no equivalent field anywhere in the schema.
WORKED: Reused the EXISTING `badge-pending`/`badgePending` infrastructure (built for `registration_code_pending_since`, new-school submissions) rather than inventing new styling — its Malay copy "Dalam Proses Menunggu Kelulusan KPM" already matched what was needed. Split the design by data reality: JKM auto-computed (`jkm_valid_to < now()`), KPM manual-only via a new boolean flag (`schools.kpm_renewal_pending`, admin-set) since no expiry date exists to compute from. Extended the existing badge if/else precedence chain (registration_code_pending_since → jkmLicenseExpired → kpmRenewalPending → normal category badge) rather than a parallel code path.
FAILED: Considered auto-detecting KPM staleness from `last_registry_verified_at` (item 10 in CLAUDE.md §2.6) — rejected because that column tracks "when we last confirmed this row", not "is the license itself expired", and would produce false pendings for schools nobody has re-synced recently but whose license is fine.
RULE: When a policy needs symmetric treatment across JKM and KPM but the schema is asymmetric (one has a real expiry date, one doesn't), don't force a fake symmetric mechanism — build auto-detection where the data supports it and an explicit manual flag where it doesn't, and say so in a comment at the point of asymmetry.
ROUTED TO: CLAUDE.md §2.6 (new item, JKM/KPM expiry-data asymmetry + `kpm_renewal_pending`); carischool-data-layer skill (schema fact); carischool-page-builder skill (badge-reuse/precedence-chain pattern). Standing policy also appended to /projects/.../ways-of-working.md this session.
